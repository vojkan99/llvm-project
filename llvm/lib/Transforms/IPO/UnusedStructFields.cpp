  ...
  for (const auto &GV : M.getGlobalList())
    if (isa<GlobalVariable>(GV) &&
      ...
      if (auto *ST = dyn_cast<StructType>(
              GV.getType()->getNonOpaquePointerElementType())) {
        // GV.dump();
...
void fixArgumentAttributes(Function *OldFunc, Function *NewFunc,
  ...
  for (const Argument &A : OldFunc->args()) {
    ...
    // is of pointer type to that structure with the appropriate
    // attribute (byval, preallocated, inalloca, byref, sret) which
    // has a type argument, which is the same as the pointee type of
    // the argument.
    if (A.hasPointeeInMemoryValueAttr())
      if (auto *OldPT = dyn_cast<PointerType>(A.getType()))
        if (auto *OldST = dyn_cast<StructType>(OldPT->getPointerElementType()))
          if (OldST->getName().compare(StructName) == 0) {
            AttributeSet ParamAttrs =
                OldFunc->getAttributes().getParamAttrs(A.getArgNo());
            ...
            if (B.getByValType())
              B.addByValAttr(NewStruct);
            if (B.getPreallocatedType())
              B.addPreallocatedAttr(NewStruct);
            if (B.getInAllocaType())
              B.addInAllocaAttr(NewStruct);
            if (B.getByRefType())
              B.addByRefAttr(NewStruct);
            if (B.getStructRetType())
              B.addStructRetAttr(NewStruct);
    ...
    NewArgAttrs.push_back(OldFunc->getAttributes().getParamAttrs(A.getArgNo()));
  }
    ...
    if (CallBase *CB = dyn_cast<CallBase>(U.getUser())) {
      for (unsigned ArgNo = 0; ArgNo < F->getFunctionType()->getNumParams();
        ...
        if (F->getArg(ArgNo)->hasPointeeInMemoryValueAttr())
          if (auto *PT = dyn_cast<PointerType>(F->getArg(ArgNo)->getType()))
            if (auto *ST = dyn_cast<StructType>(PT->getPointerElementType()))
              if (ST->getName().compare(StructName) == 0) {
                AttributeSet ParamAttrs =
...
UnusedStructureFieldsEliminationPass::run(Module &M, ModuleAnalysisManager &) {
  ...
  for (const auto &mapEntry : FieldUsage) {
    ...
    for (unsigned Cnt = 0; Cnt < M.getFunctionList().size(); Cnt++) {
      ...
      if (!Func.isDeclaration()) {
        ...
        for (const Argument &A : Func.args())
          ...
          } else if (auto *OldPT = dyn_cast<PointerType>(A.getType())) {
            if (auto *OldST =
                    dyn_cast<StructType>(OldPT->getPointerElementType()))
              if (OldST->getName().compare(StructName) == 0) {
                ArgTypes.push_back(
                    PointerType::get(NewStruct, OldPT->getAddressSpace()));
                IsNewTy = true;
                continue;
              }
          } else
        ...
        // New structure's name has not yet been changed to be the name of the
        // old structure, so the temporary name (with "_1") is passed here.
        fixCallBasesIfNecessary(NewFunc, StructName + "_1");
      }
    }
    errs() << "Out\n";
    ...
    // current one is eliminated.
    for (auto &Var : make_early_inc_range(M.getGlobalList()))
      ...
      if (isa<GlobalVariable>(Var))
        if (auto *TempType = dyn_cast<StructType>(
          ...
          if (TempType->getName().compare(StructName) == 0) {
            ...
            Var.replaceAllUsesWith(NewGlobalVar, true);
            Var.eraseFromParent();
      ...
      for (BasicBlock &BB : F)
        for (Instruction &I : make_early_inc_range(BB))
          if (auto *AllocationInst = dyn_cast<AllocaInst>(&I)) {
            ...
            if (auto *AIST =
              ...
              if (AIST->getName().str().compare(StructName) == 0) {
                ...
                AllocationInst->replaceAllUsesWith(NewAllocationInst, true);
          ...
          // size of the new struct.
            ...
            // else
            //   errs() << "Are not same\n";
            if (MCInst->getSource()->getType() == MCInst->getDest()->getType())
              if (auto *OldMC =
                      dyn_cast<PointerType>(MCInst->getSource()->getType()))
                if (auto *MCType =
                        dyn_cast<StructType>(OldMC->getPointerElementType())) {
                  if (MCType->getName().str().compare(StructName + "_1") == 0) {
                    // MCInst->dump();
                    DataLayout DL(&M);
                    uint64_t Offset = 0;
                    uint64_t MaxAlignInBits = 0;
