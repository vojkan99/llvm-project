; RUN: opt -iroutliner -ir-outlining-no-cost -S < %s | FileCheck %s
; RUN: opt -passes=iroutliner -ir-outlining-no-cost -S < %s | FileCheck %s

; RUN: opt -iroutliner -ir-outlining-no-cost < %s | \
...
; CHECK-SAME: type: ![[#OUTLINED_SUBROUTINE0+1]], flags: DIFlagOutlined,
; CHECK-SAME: spFlags: DISPFlagDefinition | DISPFlagOptimized, unit: !0,
; CHECK-SAME: retainedNodes: !{{[1-9][0-9]+}})
